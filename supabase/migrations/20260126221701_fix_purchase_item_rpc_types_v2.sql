-- ============================================
-- 🏛️ RENAISSANCE MASTER SCHEMA: RPC FIX V2
...
BEGIN
    -- 1. Parse and validate item_id (UUID format)
    BEGIN
    ...
    -- item_id is text in profile_inventory, so we cast v_item_id to text
    SELECT COALESCE(SUM(purchase_count), 0) INTO v_owned_count
    FROM public.profile_inventory
    WHERE profile_id = p_profile_id AND item_id = v_item_id::TEXT;
    ...
    VALUES (p_profile_id, v_item_id::TEXT, 1)
