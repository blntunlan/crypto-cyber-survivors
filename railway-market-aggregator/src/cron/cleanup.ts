    ...
    try {
      ...
      try {
        ...
        do {
          ...
          const mraRow = mraResult.rows[0] as
            | { cleanup_old_market_runtime_audit?: string }
            | undefined;
        ...
        if (mraDeleted > 0) {
          ...
          Logger.info(
            `[Cleanup] Deleted ${mraDeleted} old market_runtime_audit records`
          );
        }
      } catch {
        Logger.debug(
          '[Cleanup] cleanup_old_market_runtime_audit function not yet available'
        );
      }
      ...
      try {
        ...
        const auditEvRow = auditEvResult.rows[0] as
          | { cleanup_old_audit_events?: string }
          | undefined;
      ...
      try {
        ...
        const telemetryRow = telemetryResult.rows[0] as
          | { cleanup_old_product_telemetry_events?: string }
          | undefined;
        const telemetryDeleted = Number(
          telemetryRow?.cleanup_old_product_telemetry_events ?? 0
        );
        if (telemetryDeleted > 0) {
          totalDeleted += telemetryDeleted;
          Logger.info(
            `[Cleanup] Deleted ${telemetryDeleted} old product_telemetry_events`
          );
        }
      } catch {
        Logger.debug(
          '[Cleanup] cleanup_old_product_telemetry_events function not yet available'
        );
      }

      // 8. Audit log (90-day retention — legacy table; only this cron actually runs)
      try {
        const auditResult = await db.execute(
          sql`SELECT cleanup_old_audit_logs(90, ${BATCH_SIZE})`
        );
        const auditRow = auditResult.rows[0] as
          | { cleanup_old_audit_logs?: string }
          | undefined;
