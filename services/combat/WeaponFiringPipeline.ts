...
function findNearestEnemy(
  ...
  enemyGrid.forEachInRangeWithContext(playerX, playerY, 1, TARGETING_CONTEXT, checkEnemyContext);

  // Step 2: SpatialGrid 7x7 (extended range)
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
  if (!TARGETING_CONTEXT.found) {
    enemyGrid.forEachInRangeWithContext(playerX, playerY, 3, TARGETING_CONTEXT, checkEnemyContext);
  }
  ...
  if (!TARGETING_CONTEXT.found) {
    ...
    for (let i = 0; i < enemies.length; i++) {
      ...
      checkEnemyContext(enemy, TARGETING_CONTEXT);
    }
  }
