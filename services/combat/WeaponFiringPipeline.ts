...
interface TargetingContext {
  playerX: number;
  playerY: number;
  viewportBounds: ReturnType<typeof createViewportBounds> | null;
  bestX: number;
  bestY: number;
  bestDistSq: number;
  bestSpeed: number;
...
/**
 * Static context for targeting to prevent closure allocations during grid queries.
 * Safe to use globally because targeting queries are strictly synchronous and non-reentrant.
 */
...
function findNearestEnemy(
  ...
  enemyGrid.forEachInRangeWithContext(playerX, playerY, 1, TARGETING_CONTEXT, handleTargetingEnemy);

  // Step 2: SpatialGrid 7x7 (extended range)
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
  if (!TARGETING_CONTEXT.found) {
    enemyGrid.forEachInRangeWithContext(playerX, playerY, 3, TARGETING_CONTEXT, handleTargetingEnemy);
  }
  ...
  if (!TARGETING_CONTEXT.found) {
    ...
    for (let i = 0; i < enemies.length; i++) {
      handleTargetingEnemy(enemies[i]!, TARGETING_CONTEXT);
    }
  }
