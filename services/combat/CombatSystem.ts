...
// Static context and function for zero-allocation spatial grid queries
interface TargetingContext {
  playerX: number;
  playerY: number;
  viewportBounds: { left: number; right: number; top: number; bottom: number } | null;
  bestCandidate: { x: number; y: number; distSq: number; speed: number } | null;
}
...
function evaluateEnemyCandidate(
  enemy: Enemy & { isDying?: boolean },
  ...
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
  if (!ctx.bestCandidate || distSq < (ctx.bestCandidate as {distSq: number}).distSq) {
...
export class CombatSystem implements ICombatSystem {
  ...
  private findNearestEnemy(
    ...
    enemyGrid.forEachInRangeWithContext(player.x, player.y, 1, TARGETING_CONTEXT, evaluateEnemyCandidate);

    // Step 2: If nothing found, check 7x7 grid (extended surroundings)
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    if (!TARGETING_CONTEXT.bestCandidate) {
      enemyGrid.forEachInRangeWithContext(player.x, player.y, 3, TARGETING_CONTEXT, evaluateEnemyCandidate);
    }
    ...
    if (!TARGETING_CONTEXT.bestCandidate) {
      ...
      for (let i = 0, len = enemies.length; i < len; i++) {
        ...
        evaluateEnemyCandidate(enemy, TARGETING_CONTEXT);
      }
    }

    const bestCandidate = TARGETING_CONTEXT.bestCandidate;
    if (!bestCandidate) return null;
    const finalCandidate = bestCandidate as { x: number; y: number; distSq: number; speed: number };
    return {
      x: finalCandidate.x,
      y: finalCandidate.y,
      dist: Math.sqrt(finalCandidate.distSq),
      speed: finalCandidate.speed,
    };
  }
