...
export class CombatSystem implements ICombatSystem {
  ...
  private static readonly TARGETING_CONTEXT = {
    ...
    bestEnemy: null as Enemy | null,
    bestDistSq: Number.MAX_VALUE,
  };

  private static handleTargetingCandidate(
    enemy: Enemy,
  ...
  private findNearestEnemy(
    ...
    enemyGrid.forEachInRangeWithContext(
      ...
      1,
      ctx,
      CombatSystem.handleTargetingCandidate
    );
    ...
    if (!ctx.bestEnemy) {
      ...
      for (let i = 0, len = enemies.length; i < len; i++) {
        ...
        CombatSystem.handleTargetingCandidate(enemy, ctx);
      }
    }

    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    const result = ctx.bestEnemy
      ? {
          x: ctx.bestEnemy.x,
          y: ctx.bestEnemy.y,
          dist: Math.sqrt(ctx.bestDistSq),
          speed: ctx.bestEnemy.speed,
        }
      : null;
