...
const TARGETING_CONTEXT: TargetingContext = {
  ...
  viewportBounds: null,
  bestDistSq: Infinity,
  bestCandidate: { x: 0, y: 0, distSq: Infinity, speed: 0 }, // Pre-allocated to avoid object creation
};

function processEnemyCandidate(enemy: Enemy, ctx: TargetingContext): void {
...
export class CombatSystem implements ICombatSystem {
  ...
  private findNearestEnemy(
    ...
    // Signal no valid candidate yet by using Infinity for distSq
    if (TARGETING_CONTEXT.bestCandidate) TARGETING_CONTEXT.bestCandidate.distSq = Infinity;
    ...
    enemyGrid.forEachInRangeWithContext(
      ...
      1,
      TARGETING_CONTEXT,
      processEnemyCandidate
    );

    // Step 2: If nothing found, check 7x7 grid (extended surroundings)
    if (TARGETING_CONTEXT.bestDistSq === Infinity) {
      enemyGrid.forEachInRangeWithContext(
        player.x,
        player.y,
        3,
    ...
    let bestCandidate = TARGETING_CONTEXT.bestDistSq !== Infinity ? { ...TARGETING_CONTEXT.bestCandidate! } : null;
