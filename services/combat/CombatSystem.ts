...
export class CombatSystem implements ICombatSystem {
  ...
  private static handleTargetingNeighbor(
    enemy: Enemy,
  ...
  private findNearestEnemy(
    ...
    // Step 1: Check 3x3 grid (immediate surroundings) using zero-allocation context variant
    enemyGrid.forEachInRangeWithContext(
      player.x,
      player.y,
      1,
      ctx,
      CombatSystem.handleTargetingNeighbor
    );
    ...
    if (!ctx.found) {
      ...
      for (let i = 0, len = enemies.length; i < len; i++) {
        ...
        if (enemy !== undefined) {
          CombatSystem.handleTargetingNeighbor(enemy, ctx);
        }
      }
    }
