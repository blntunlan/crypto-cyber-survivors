...
export class MovementSystem implements IMovementSystem {
  ...
  private updateEnemies(
    ...
    // ⚡ Bolt: Replace forEach with for loop to eliminate closure allocation and GC pressure
  ...
  private updateImpactRings(pool: IPoolManager, dtFactor: number): void {
    // ⚡ Bolt: Replace forEach with for loop to eliminate closure allocation and GC pressure
    const impactRings = pool.activeImpactRings;
    for (let i = 0, len = impactRings.length; i < len; i++) {
  ...
  private updateFloatingTexts(pool: IPoolManager, dtFactor: number): void {
    // ⚡ Bolt: Replace forEach with for loop to eliminate closure allocation and GC pressure
    const floatingTexts = pool.activeFloatingTexts;
