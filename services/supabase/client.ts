...
if (!isConfigured) {
  console.warn(
    '⚠️ [WARN] [Supabase] Missing credentials! Backend features are DISABLED. Check VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.'
  );
} else {
  if (import.meta.env.DEV) {
    // eslint-disable-next-line no-console
    console.info(`ℹ️ [INFO] [Supabase] Initializing connection to project: ${supabaseUrl}`);
  }
}
