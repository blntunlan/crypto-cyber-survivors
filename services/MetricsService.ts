...
class MetricsServiceClass {
    ...
    private compileBitcoinMetrics(finalData: {
        ...
        const priceChange = finalData.entryPrice > 0
    ...
    private compileCardMetrics(): CardMetrics {
        ...
        const avgTimeToLevelUp = levelUpTimes.length > 0
    ...
    private handleQuotaExceeded(): void {
        ...
        Logger.info(
            `[Metrics] Removed ${originalCount - keepCount} old sessions, kept ${keepCount}`
        );
    ...
    getPlayerExperienceInsights(): PlayerExperienceInsights {
        ...
        const avgLevelsPerMinute =
            avgDuration > 0
                ? (sessions.reduce((a, b) => a + b.player.maxLevel, 0) / sessions.length) /
                (avgDuration / 60)
                : 0;
    ...
    private generateRecommendations(): string[] {
        ...
        if (insights.difficulty.nearDeathUsage.avgPerGame < 0.5) {
            recommendations.push('Near-death mercy sistemi nadiren kullanılıyor. Eşiği yükseltmeyi düşün.');
        }
        ...
        if (Math.abs(insights.bitcoin.pnlDifficultyCorrelation) < 0.3) {
            recommendations.push('Bitcoin fiyatının zorluk üzerinde etkisi düşük. PnL faktörünü güçlendir.');
        }
