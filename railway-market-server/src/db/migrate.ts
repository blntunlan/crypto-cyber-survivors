...
const MIGRATION_015 = `
-- Migration 015: server-authoritative cash-out economy
...
CREATE TABLE IF NOT EXISTS run_escrows (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT ck_run_escrows_state
    CHECK (state IN ('active', 'quote_open', 'settled', 'failed')),
...
CREATE TABLE IF NOT EXISTS cash_out_quotes (
  ...
  idempotency_key TEXT,
  CONSTRAINT ck_cash_out_quotes_status
    CHECK (status IN ('open', 'accepted', 'rejected', 'expired', 'safe_exit')),
...
const MIGRATION_016 = `
-- Migration 016: immutable primary reward-point settlement ledger
