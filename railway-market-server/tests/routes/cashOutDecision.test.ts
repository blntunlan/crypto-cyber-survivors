...
describe('cash-out decision route', () => {
  ...
  it('records an expired timeout as Greed', async () => {
    const expiredIssuedAt = ISSUED_AT - 30;
    const expiredAt = expiredIssuedAt + 15;
    const expiredSignature = crypto
      .createHmac('sha256', SECRET)
      .update([QUOTE_ID, SESSION_ID, 42, 120, expiredIssuedAt, expiredAt].join(':'))
      .digest('hex');
    const client = {
      query: vi
        .fn()
        ...
        .mockResolvedValueOnce({
          rows: [{
            escrow_id: 'escrow-1', session_id: SESSION_ID, quote_id: QUOTE_ID,
            canonical_sequence: 42, reward_points: 120, signature: expiredSignature,
            issued_at: new Date(expiredIssuedAt * 1_000), expires_at: new Date(expiredAt * 1_000),
            status: 'open', greed_level: 2,
          }],
        })
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [] })
    ...
    expect(client.query.mock.calls.some(([statement, values]) =>
      String(statement).includes('UPDATE cash_out_quotes') && values?.[0] === 'expired'
    )).toBe(true);
  });

  it('rejects timeout before the signed expiry', async () => {
    const client = {
      query: vi
        .fn()
        .mockResolvedValueOnce({ rows: [{ id: PROFILE_ID }] })
    ...
    expect(response.body).toEqual({ error: 'Cash-out quote has not expired' });
    expect(client.query).toHaveBeenCalledTimes(3);
  });
});
