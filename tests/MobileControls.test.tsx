...
import { type MobileControlSettings } from '../types/MobileSettings';
...
vi.mock('../components/mobile/VirtualJoystick', () => ({
  VirtualJoystick: ({ onMove }: { onMove: (x: number, y: number) => void }) => (
    <div data-testid="virtual-joystick" onClick={() => onMove(1, 0)}>
      Joystick
    </div>
  ),
}));
...
vi.mock('../components/mobile/DashButton', () => ({
  DashButton: ({ onDash }: { onDash: () => void }) => (
    <button data-testid="dash-button" onClick={onDash}>
      Dash
    </button>
  ),
}));
