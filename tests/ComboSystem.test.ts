...
describe('ComboSystem', () => {
  ...
  // Helper to advance game time (handles TimeService's 50ms delta cap)
  const advanceGameTime = (totalMs: number) => {
    const step = 50; // TimeService caps delta at 50ms
  ...
  it('should reset combo after timeout', () => {
    ...
    // Advance game time past timeout (3000ms + buffer for 50ms cap granularity)
