...
describe('RSICalculator', () => {
  ...
  describe('Initialization', () => {
    ...
    it('should be initialized after enough data points', () => {
      // Need period + 1 data points (7 + 1 = 8)
      for (let i = 0; i < 8; i++) {
  ...
  describe('RSI Calculation', () => {
    ...
    it('should return 100 when only gains (all prices increasing)', () => {
      // Start with enough history, then all gains
    ...
    it('should return 0 when only losses (all prices decreasing)', () => {
      ...
      const prices = [108, 107, 106, 105, 104, 103, 102, 101, 100];
      prices.forEach(p => calculator.update(p));
    ...
    it('should calculate RSI correctly for mixed price changes', () => {
      // Mix of gains and losses - more balanced
  ...
  describe('RSI State Detection', () => {
    it('should detect OVERSOLD state when RSI < 30', () => {
      // Strong downtrend to get RSI below 30
      const prices = [100, 95, 90, 85, 80, 75, 70, 65, 60];
      prices.forEach(p => calculator.update(p));
    ...
    it('should detect OVERBOUGHT state when RSI > 70', () => {
      // Strong uptrend to get RSI above 70
      const prices = [100, 105, 110, 115, 120, 125, 130, 135, 140];
      prices.forEach(p => calculator.update(p));
    ...
    it('should detect NEUTRAL state for moderate RSI', () => {
      ...
      const prices = [100, 102, 100, 102, 100, 102, 100, 102, 100];
      prices.forEach(p => calculator.update(p));
  ...
  describe('Hysteresis (Flickering Prevention)', () => {
    it('should stay OVERSOLD when RSI rises to 31 (below exit threshold of 35)', () => {
      ...
      calculator.update(61);
      calculator.update(62);
    ...
    it('should exit OVERSOLD when RSI rises above 35 (exit threshold)', () => {
      // First, get into OVERSOLD state with strong downtrend
      const downPrices = [100, 95, 90, 85, 80, 75, 70, 65, 60];
      downPrices.forEach(p => calculator.update(p));
      expect(calculator.getState()).toBe('OVERSOLD');

      // Moderate recovery - small gains to push RSI above 35 but not above 70
      // Using smaller increments to avoid going to OVERBOUGHT
      calculator.update(61);
      calculator.update(62);
      calculator.update(63);
      calculator.update(64);
      calculator.update(65);
      calculator.update(66);
      calculator.update(67);

      // Since the price was heavily down before, a moderate recovery
      // should bring RSI to NEUTRAL range
      // Note: The exact behavior depends on the calculation window
      const rsi = calculator.getRSI();
      ...
      // The key is that hysteresis is working
    ...
    it('should stay OVERBOUGHT when RSI drops to 66 (above exit threshold of 65)', () => {
      // First, get into OVERBOUGHT state
      ...
      calculator.update(178);
      calculator.update(176);
  ...
  describe('State Change Detection', () => {
    it('should detect when state changes', () => {
      ...
      // Update with neutral prices to build history
      const neutralPrices = [100, 101, 100, 101, 100, 101, 100, 101];
      ...
      const downPrices = [99, 95, 90, 85, 80, 75, 70, 65, 60];
      downPrices.forEach(p => calculator.update(p));

      // State should now be OVERSOLD
      expect(calculator.getState()).toBe('OVERSOLD');

      // didStateChange checks if current update changed state,
      // but we're checking after many updates.
      // So we verify the end state is different from start
  ...
  describe('Reset', () => {
    it('should reset all state', () => {
      // Build up some state
      const prices = [100, 90, 80, 70, 60, 50, 40, 30, 20];
      prices.forEach(p => calculator.update(p));

      expect(calculator.getState()).toBe('OVERSOLD');
      expect(calculator.getHistoryLength()).toBe(9);
