...
describe('CullingUtils', () => {
  ...
  describe('updateViewportBounds', () => {
    it('should update bounds in-place with correct padding', () => {
      const boundsToUpdate = createViewportBounds(0, 0, 0);
      updateViewportBounds(boundsToUpdate, 800, 600, 50);

      expect(boundsToUpdate).toEqual({
        left: -50,
        right: 850,
        top: -50,
        bottom: 650,
    ...
    it('should use default padding if not provided', () => {
      const boundsToUpdate = createViewportBounds(0, 0, 0);
      updateViewportBounds(boundsToUpdate, 800, 600);

      expect(boundsToUpdate.left).toBe(-50);
    });
  });
