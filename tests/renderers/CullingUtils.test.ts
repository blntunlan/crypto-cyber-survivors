...
describe('CullingUtils', () => {
  ...
  describe('updateViewportBounds', () => {
    it('should modify existing bounds object in-place correctly', () => {
      const targetBounds = { left: 0, right: 0, top: 0, bottom: 0 };
      const returnedBounds = updateViewportBounds(targetBounds, 800, 600, 50);

      expect(targetBounds).toBe(returnedBounds); // Exact same object reference
