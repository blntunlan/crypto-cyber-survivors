...
describe('CullingUtils', () => {
  ...
  describe('updateViewportBounds', () => {
    it('should update properties in-place', () => {
      const myBounds = { left: 0, right: 0, top: 0, bottom: 0 };
      updateViewportBounds(myBounds, 800, 600, 50);

      expect(myBounds).toEqual({
        left: -50,
        right: 850,
        top: -50,
        bottom: 650,
    ...
    it('should use default padding if not provided', () => {
      const myBounds = { left: 0, right: 0, top: 0, bottom: 0 };
      updateViewportBounds(myBounds, 800, 600);

      expect(myBounds.left).toBe(-50);
    });
  });
