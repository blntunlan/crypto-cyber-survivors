...
const QualitySection = memo(() => {
  const [currentProfile, setCurrentProfile] = React.useState(
    DeviceBenchmarkService.getPerformanceConfig().profile
  );
  // Track if we are in "Auto" mode (matching benchmark result)
  const [isAuto, setIsAuto] = React.useState<boolean>(true);
  ...
  React.useEffect(() => {
    const updateState = () => {
      ...
      // If cached config profile matches benchmark result profile, we consider it Auto
      // (Strictly speaking, manual override could be set to same value, but that's fine)
