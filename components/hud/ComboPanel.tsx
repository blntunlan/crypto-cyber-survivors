...
const MobileComboPanel: React.FC<ComboPanelProps> = ({ containerRef, maxStreak, totalBonusXp }) => (
  <div
    ref={containerRef}
    className="absolute bottom-32 left-1/2 z-[115] bg-transparent p-2 min-w-[130px] transition-all duration-300 ease-out flex flex-col items-center pointer-events-none"
    style={{
      opacity: 0,
      transform: 'translateX(-50%) translateY(20px)',
      willChange: 'transform, opacity',
  ...
  >
    ...
    <div className="w-full px-1">
      ...
      <div className="flex items-center justify-center gap-1.5 drop-shadow-[0_0_10px_rgba(0,0,0,1)]">
        <span
          id="combo-streak-count"
          className="text-4xl font-black italic tracking-tighter text-white tabular-nums"
          style={{ textShadow: '0 0 20px rgba(0,0,0,1), 0 0 10px rgba(255,255,255,0.3)' }}
        >
          0
        </span>
      ...
      <div
        id="combo-multiplier-badge"
        className="mt-1 text-cyan-400 font-black italic text-center text-xs drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]"
      >
        1.0x XP
      </div>
    </div>
