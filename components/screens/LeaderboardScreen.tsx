  ...
  return useMemo(
    () => [
      ...
      },
      {
        ...
        icon: <IconTimeLock className="h-3.5 w-3.5" />,
      ...
      },
      {
        ...
        icon: <IconSkull className="h-3.5 w-3.5" />,
...
const PodiumCard: React.FC<PodiumCardProps> = ({
  ...
  return (
    <div
      className={cn(
        'screen-fade-in relative flex flex-col items-center gap-3 p-4 text-center',
        isRetro
          ? 'border-2 bg-[#0a0a12]/80'
          : `rounded-lg border bg-gradient-to-b ${config.gradient} ${config.border} ${config.glow} backdrop-blur-sm`
      )}
      style={{
        animationDelay: `${animDelay}s`,
        ...(isRetro ? { borderColor: sortTab.color + '60' } : undefined),
      }}
    >
      {/* Rank Badge */}
      <div className="relative">
        ...
        >
          <Icon
            ...
            color={isRetro ? sortTab.color : 'currentColor'}
          />
        </div>
      ...
      <div className="w-full min-w-0">
        ...
        {entry.isCurrentPlayer && (
          ...
          >
            {(t('leaderboard.you') as string) || 'YOU'}
          </span>
        )}
      </div>
...
const RankRow: React.FC<RankRowProps> = ({ entry, sortTab, isRetro, animDelay }) => {
  ...
  return (
    <div
      className={cn(
        'screen-fade-in group relative flex items-center gap-3 px-4 py-3 transition-all',
        entry.isCurrentPlayer
          ? isRetro
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-3">
          <span className="flex items-center gap-1 text-[10px] text-slate-500">
            <IconTimeLock className="h-3 w-3" />
      ...
      <div className="relative">
        ...
        {!isRetro && (
          <div className="absolute inset-0 animate-pulse blur-lg">
            <IconTrophy className="h-12 w-12 text-yellow-400/20" />
          </div>
        )}
      </div>
...
export const LeaderboardScreen: React.FC = () => {
  ...
  return (
    <div
      className={cn(
        'screen-fade-in allow-scroll absolute inset-0 z-[100] flex flex-col items-center justify-start overflow-y-auto p-3 pb-[calc(1rem+var(--sab))] pt-[calc(0.75rem+var(--sat))] sm:p-5',
        isRetro ? 'bg-[#0a0a12]/95' : 'bg-slate-950/95'
      )}
    >
      <div className="w-full max-w-2xl space-y-4 py-2 sm:space-y-5 sm:py-4">
        {/* Header */}
        <header className="screen-fade-in space-y-2 text-center">
          <div className="flex items-center justify-center gap-3">
            <IconTrophy
              className={cn('h-5 w-5 text-yellow-400', !isRetro && 'animate-pulse')}
            />
            <h1
              className={cn(
                isRetro
        ...
        <div
          className={cn(
            'screen-fade-in flex gap-1.5 overflow-x-auto p-1',
            isRetro
        ...
        >
          {sortTabs.map(tab => (
            <button
              ...
              className={cn(
                'flex min-h-[44px] flex-1 touch-manipulation items-center justify-center gap-1.5 whitespace-nowrap px-3 py-2.5 text-[10px] font-bold uppercase tracking-[0.15em] transition-all duration-200 active:scale-95',
                isRetro
                  ? activeSort === tab.key
                    ? 'border-2 bg-[#0a0a12]'
          ...
          {loading && entries.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-16">
              ...
              <ThemedText className="text-xs text-slate-500">
                {(t('leaderboard.loading') as string) || 'Loading rankings...'}
              </ThemedText>
            </div>
          ) : entries.length === 0 ? (
            ...
            <div className="relative z-10">
              ...
              <div
                className={cn(
                  'flex items-center justify-between border-t px-4 py-1.5',
              ...
              >
                ...
                <button
                  ...
                  disabled={refreshing}
                  className={cn(
                    'flex min-h-[44px] touch-manipulation items-center gap-1.5 rounded px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-[0.15em] transition-all active:scale-95',
                    isRetro
                      ? 'border border-[#39FF14]/30 text-[#39FF14] hover:bg-[#39FF14]/10'
                      : 'text-slate-400 hover:bg-white/10 hover:text-white'
                  )}
                >
                  <IconRepeat className={cn('h-3 w-3', refreshing && 'animate-spin')} />
                  {refreshing
