...
function getRequiredExp(level) {
  if (level < CONFIG.PLATEAU_LEVEL) {
    ...
    const plateauExp = Math.floor(
      CONFIG.BASE_EXP + 
      Math.pow(CONFIG.PLATEAU_LEVEL - 1, CONFIG.CURVE_EXPONENT) * 40
    );
...
function runSimulation() {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    console.log(`Level ${String(i).padStart(2, ' ')} -> ${String(i + 1).padStart(2, ' ')}: ${String(Math.round(nextLevelExp)).padStart(5, ' ')} EXP (~${String(gemsNeeded).padStart(4, ' ')} gems)${marker}`);
