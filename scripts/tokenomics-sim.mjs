...
function weeklyEmission(pop, y) {
  ...
  const rewards = pop.map((p) => (p.eff > 0 && sumEff > 0 ? (bWeek * p.eff) / sumEff : 0));
  return { bWeek, bMax, clampActive, qualifiedCount: qualified.length, rewards };
}

// helpers
const fmt = (n) => (n >= 1e6 ? (n / 1e6).toFixed(3) + "M" : n >= 1e3 ? (n / 1e3).toFixed(1) + "k" : n.toFixed(1));
const pad = (s, w) => String(s).padStart(w);
function pct(arr, p) { const s = [...arr].sort((a, b) => a - b); return s.length ? s[Math.floor((s.length - 1) * p)] : 0; }
...
console.log(`B_max(hafta)=${fmt(bMaxWeek(1))}  r_target=${rTarget(1).toFixed(0)}  clamp esigi ~${Math.round(bMaxWeek(1) / rTarget(1))} oyuncu  (bot orani %${CONFIG.botShare * 100})\n`);
console.log([pad("oyuncu", 7), pad("bWeek", 9), pad("clamp", 6), pad("ort/oyuncu", 11), pad("medyan(L)", 10), pad("top1%(L)", 9), pad("medyan(bot)", 12), pad("bot/legit", 10)].join(" |"));
for (const n of [500, 1500, 5000, 20000]) {
  ...
  const mL = median(legitR), mB = median(botR);
  console.log([pad(n, 7), pad(fmt(res.bWeek), 9), pad(res.clampActive ? "EVET" : "hayir", 6), pad(avg.toFixed(0), 11), pad(mL.toFixed(0), 10), pad(pct(legitR, 0.99).toFixed(0), 9), pad(mB.toFixed(1), 12), pad((mL ? (mB / mL) * 100 : 0).toFixed(1) + "%", 10)].join(" |"));
}
...
console.log([pad("yil", 3), pad("oyuncu", 7), pad("r_target", 8), pad("B_max/hf", 9), pad("gercek/hf", 9), pad("clamp", 6), pad("yillik", 9), pad("kumulatif", 10)].join(" |"));
let cumulative = 0;
for (let y = 1; y <= CONFIG.years; y++) {
  const players = trajectory[y - 1];
  const rt = rTarget(y), bMax = bMaxWeek(y);
  const bWeek = Math.min(bMax, rt * players);
  const annual = bWeek * CONFIG.weeksPerYear;
  cumulative += annual;
  console.log([pad(y, 3), pad(players, 7), pad(rt.toFixed(0), 8), pad(fmt(bMax), 9), pad(fmt(bWeek), 9), pad(rt * players >= bMax ? "EVET" : "hayir", 6), pad(fmt(annual), 9), pad(fmt(cumulative), 10)].join(" |"));
}
console.log(`Havuz=${fmt(CONFIG.emissionPool)} | 5y kumulatif=${fmt(cumulative)} | kalan=${fmt(CONFIG.emissionPool - cumulative)} | havuz omru: ${cumulative <= CONFIG.emissionPool ? ">= 5 yil" : "< 5 yil (clamp surekli aktif)"}`);
...
console.log([pad("harcama", 8), pad("yakim payi", 11), pad("hf yakim", 9), pad("net enflasyon", 14), pad("sink/faucet", 11)].join(" |"));
for (const spendRate of [0.3, 0.5, 0.8, 1.2]) {
  const burnShare = 0.7;
  const weeklyBurn = weeklyEmit * spendRate * burnShare;
  const net = weeklyEmit - weeklyBurn;
  console.log([pad((spendRate * 100) + "%", 8), pad((burnShare * 100) + "%", 11), pad(fmt(weeklyBurn), 9), pad(fmt(net), 14), pad((weeklyBurn / weeklyEmit).toFixed(2), 11)].join(" |"));
}
console.log("Not: sink/faucet>=1 icin harcama x yakim_payi >= 1. Saf kozmetik (%70 yakim) tek basina yetmez;");
console.log("     itibar/prestij sink'i (%100 yakim) ile kapatilir -> bu yuzden status sink'i kritik.");
